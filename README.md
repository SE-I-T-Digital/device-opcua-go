...
The `Attributions.txt` file can be generated by running

```bash
make attributions
```

This will install [`github.com/google/go-licenses/v2`](https://github.com/google/go-licenses/) and run it against the repository contents, then report any missing attributions using `./bin/test-attribution-txt.sh`.

### Attributions behind build tags
